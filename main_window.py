...
from PyQt5.QtWidgets import (
    ...
    QDoubleSpinBox, QSpinBox, QColorDialog, QWIDGETSIZE_MAX, QDesktopWidget, qApp
)
...
class MainWindow(QMainWindow):
    ...
    def on_focus_changed(self, old, now):
        if now is None:
            ...
            self.canvas_widget.alt_pressed = False
    ...
    def export_flowmap(self):
        """处理导出流动图的功能，支持多种格式"""
        # 创建带有过滤器的文件对话框
        image_filter = f"{translator.tr('tga_files')};;{translator.tr('png_files')};;{translator.tr('jpg_files')};;{translator.tr('bmp_files')};;{translator.tr('exr_files')}"
